export default class StepSlider {
  constructor({ steps, value = 0 }) {
    ...
    //
    this.stepsSpan();
    this.elem.addEventListener("pointerdown", (event) => this.mouseDown(event));
  ...
  mouseDown(event) {
    ...
    thumb.addEventListener("pointermove", mouseMove);
    thumb.addEventListener("pointerup", mouseUp);

    function mouseMove() {
      // let left = event.clientX - this.elem.getBoundingClientRect().left; - Выдает ошибку , А this.elem получаю андефайн
      console.log(this.elem);
    }

    function mouseUp() {
      thumb.removeEventListener("pointerup", mouseUp);
      thumb.removeEventListener("pointermove", mouseMove);
    }
